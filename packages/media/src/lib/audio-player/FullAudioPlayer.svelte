<script lang="ts">
	...
	function handleKeyDown(event: KeyboardEvent) {
		...
		switch (event.key) {
			...
			case '<':
				event.preventDefault();
				setSpeed(Math.max(0.25, playerState.playbackSpeed - 0.25));
				break;

			case '>':
				event.preventDefault();
				setSpeed(Math.min(2, playerState.playbackSpeed + 0.25));
				break;
	...
	function handleProgressKeyDown(event: KeyboardEvent) {
		...
		event.preventDefault();
		// Mandatory, not defensive: this component also registers a `keydown`
		// listener on `window` (see onMount), whose guard only excludes inputs and
		// textareas — not this div. Without stopPropagation one arrow press seeks
		// twice, 5s here and another 5s there.
...
<div class="full-audio-player {className}" role="region" aria-label="Audio player">
	...
	{#if playerState.currentTrack}
		<div class="track-header">
			...
			<div class="track-details-large">
				<div class="track-title-large">{playerState.currentTrack.title}</div>
				{#if playerState.currentTrack.artist}
					<div class="track-artist-large">{playerState.currentTrack.artist}</div>
				{/if}
				{#if playerState.currentTrack.album}
					<div class="track-album">{playerState.currentTrack.album}</div>
				{/if}
				{#if showPlaylistInfo && playerState.playlist.length > 0}
					<div class="playlist-info">
						Track {playerState.currentTrackIndex + 1} of {playerState.playlist.length}
					</div>
				{/if}
			</div>
		</div>
	...
	<div class="progress-section">
		...
		<div class="time-display-large">
			...
			<span class="duration-large">{formatTime(playerState.duration)}</span>
		</div>
	</div>
	...
	<div class="main-controls">
		<!-- Shuffle button -->
		<button
			class="control-btn {playerState.isShuffled ? 'active' : ''}"
	...
	<Tooltip content="Skip forward 10s">
		...
		<!-- Loop mode -->
		<button
			class="control-btn {playerState.loopMode !== 'none' ? 'active' : ''}"
			onclick={toggleLoopMode}
			aria-label="Loop mode: {playerState.loopMode}"
	...
	<div class="secondary-controls">
		...
		<div class="speed-control">
			<button
				...
				aria-label="Playback speed: {playerState.playbackSpeed}x"
			>
			...
			{#if showSpeedMenu}
				<div class="speed-menu">
					{#each speedPresets as speed}
						<button
							class="speed-option {playerState.playbackSpeed === speed ? 'active' : ''}"
		...
		{#if showExpandButton}
			<button
				...
				aria-label={playerState.isExpanded ? 'Collapse' : 'Expand'}
				title={playerState.isExpanded ? 'Collapse' : 'Expand'}
			>
