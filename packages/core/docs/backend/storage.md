...
This API uses `document.cookie` and cannot set or read HttpOnly cookies.
Use server-issued HttpOnly cookies for authentication; see the
[security guide](./storage-security.md#cookie-security).
...
// Usage
setWithExpiry(storage, 'draft', { text: 'hello' }, 3600000, clock); // 1 hour
...
⚠️ **IMPORTANT**: Browser storage is NOT secure storage. See [SECURITY.md](./storage-security.md) for comprehensive security guidelines.
...
The [examples directory](https://github.com/jonathanbelolo/composable-svelte/tree/main/examples) holds complete working apps.
None is a dedicated storage demo — the closest is
[`ssr-server`](https://github.com/jonathanbelolo/composable-svelte/tree/main/examples/ssr-server), which uses `createNoopStorage()`
