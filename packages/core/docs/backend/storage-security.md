...
**JavaScript-readable browser storage is not a place for authentication secrets.** Scripts on the origin can read localStorage, sessionStorage and cookies created through `createCookieStorage`. Server-issued HttpOnly cookies are not readable through JavaScript, although XSS can still make authenticated requests.

### Attack Vectors

1. **XSS (Cross-Site Scripting)**: Malicious scripts can read JavaScript-accessible storage and make authenticated requests
...
// ✅ GOOD: Non-sensitive preference cookie
...
const preferenceStorage = createLocalStorage<Preferences>({
	prefix: 'preferences:',  // Prevents key collisions
	validator: isValidPreferences  // Runtime validation
});
...
The browser cookie adapter uses `document.cookie`; it cannot create, read or
clear an HttpOnly session cookie. Authentication belongs to the server-issued
cookie flow documented by `@composable-svelte/auth`. `Secure` restricts transport
to HTTPS; it does not prevent JavaScript access. SameSite limits cross-site
sending and does not replace backend CSRF/origin checks.
See [MDN's Set-Cookie reference](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Set-Cookie).
...
const cookies = createCookieStorage<string>({
	...
	// 2. Restrict cross-site sending
	sameSite: 'Strict',  // or 'Lax' for cross-site navigation

	// 3. Path Restriction
	path: '/app',  // Sending scope, not a security boundary
...
// Set a non-sensitive preference
cookies.setItem('theme', 'dark', {
	secure: true,
	sameSite: 'Strict',
	maxAge: 3600
...
2. **Choose SameSite for the actual flow**; the auth reference backend uses Lax to support top-level navigation
3. **Use server-issued HttpOnly cookies for sessions**, not this adapter
4. **NEVER use `sameSite: 'None'`** without `secure: true` (library enforces this)
5. **Set shortest possible `maxAge`**
6. **Treat Path as a sending filter**, not an access-control boundary

**SameSite Comparison:**

| Value    | Protection | Use Case                           |
|----------|------------|------------------------------------|
| `Strict` | Same-site requests only | Flows that do not require cross-site entry |
| `Lax` | Also allows qualifying top-level navigation | Common session/login flows |
| `None` | Allows cross-site sending | Cross-site embeds; requires Secure |
...
const profileStorage = createLocalStorage<PublicProfile>({ prefix: 'profile:' });
...
### 5. End the Server Session Before Clearing Local Data

```typescript
async function logout() {
  const response = await fetch('/auth/logout', { method: 'POST', credentials: 'include' });
  if (!response.ok) throw new Error('Logout failed');
  // The server has invalidated the session and expired its HttpOnly cookie.
  localStorage.removeItem('profile');
  sessionStorage.removeItem('draft');
  localStorage.setItem('logout-at', String(Date.now())); // Non-secret cross-tab notification
}
```
...
- [ ] Authentication cookies are server-issued and HttpOnly, with a SameSite policy suited to the login flow
