/**
 * Browser tests for Alert animation lifecycle.
 *
 * These tests verify that:
 * 1. Alert animations actually run in the browser
 ...
 * 5. Alert uses subtle scale + fade animation
 */
...
describe('Alert Animation Lifecycle', () => {
	it('should animate in when presenting', async () => {
		const { container } = render(AlertTest);

		// Get store reference from window
		const store = (window as any).__alertTestStore;
		expect(store).toBeDefined();

		// Initially, alert should not be visible
		let alertContent = document.querySelector('[data-testid="alert-content"]');
		expect(alertContent).toBeNull();

		// Click button to open alert
		const openButton = container.querySelector('[data-testid="open-alert"]') as HTMLButtonElement;
		await userEvent.click(openButton);

		// Wait for alert to mount (status changes to 'presenting')
		await waitForState(store, (state: any) => state.presentation.status === 'presenting', {
			description: "presentation status to be 'presenting'"
		});

		// Alert should now be in DOM
		alertContent = document.querySelector('[data-testid="alert-content"]');
	...
	it('should animate out when dismissing', async () => {
		...
		// Alert should be fully presented
		let alertContent = document.querySelector('[data-testid="alert-content"]');
		expect(alertContent).toBeTruthy();

		// Click dismiss button
		const dismissButton = document.querySelector('[data-testid="dismiss-alert"]') as HTMLButtonElement;
		...
		// Alert should stay mounted during dismissal animation
		alertContent = document.querySelector('[data-testid="alert-content"]');
		expect(alertContent).toBeTruthy();
		...
		// Alert should now be removed from DOM
		alertContent = document.querySelector('[data-testid="alert-content"]');
		expect(alertContent).toBeNull();
	...
	it('should prevent interactions during animation', async () => {
		...
		// Wait for alert to mount
		await waitForState(store, (state: any) => state.presentation.status === 'presenting', {
			description: "presentation to start"
		});

		// During presentation animation, button should be disabled
		let button = document.querySelector('[data-testid="alert-action-button"]') as HTMLButtonElement;
		...
		button = document.querySelector('[data-testid="alert-action-button"]') as HTMLButtonElement;
	...
	it('should handle rapid open/close transitions', async () => {
		...
		// Alert should eventually be removed
	...
	it('should animate backdrop independently', async () => {
		...
		const backdrop = document.querySelector('[data-testid="alert-backdrop"]');
		const content = document.querySelector('[data-testid="alert-content"]');
