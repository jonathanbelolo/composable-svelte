...
import type { CookieStorage, CookieConfig, CookieOptions } from './storage.js';
import {
...
/**
 * Internal registry entry for tracking cookie metadata.
 */
interface CookieRegistryEntry {
	/**
	 * Cookie options used when setting.
	 * Required for reliable removal (path/domain must match).
	 */
	options: Required<Pick<CookieOptions, 'path' | 'domain'>> &
		Partial<Pick<CookieOptions, 'secure' | 'sameSite' | 'maxAge' | 'expires'>>;
}
 ...
 * ```
 */
export function createCookieStorage<T = unknown>(
	...
	if (!navigator.cookieEnabled) {
		throw new EnvironmentNotSupportedError(
			'cookies',
			'current browser (cookies disabled)'
		);
	}
	...
	const registry = new Map<string, CookieRegistryEntry>();
	...
	function _buildCookieString(
		name: string,
		value: string,
		options: CookieRegistryEntry['options']
	): string {
	...
	function _parseCookies(): Map<string, string> {
	...
	return {
		...
		setItem(key: string, value: T, options: CookieOptions = {}): void {
			...
			const cookieOptions: CookieRegistryEntry['options'] = {
 ...
 * ```
 */
export function createMockCookieStorage<T = unknown>(
	config: CookieConfig<T> = {}
): CookieStorage<T> {
