 ...
 * ```
 */
export function connect(client, subscriptionId, url, protocols, onEvent) {
 ...
 * ```
 */
export function disconnect(client, code, reason) {
 ...
 * ```
 */
export function send(client, message, onSuccess, onFailure) {
        ...
        try {
            ...
            if (onFailure) {
                const wsError = error instanceof WebSocketError
                    ? error
                    : new WebSocketError(String(error), WS_ERROR_CODES.SEND_FAILED, true, error);
 ...
 * ```
 */
export function subscribe(client, subscriptionId, onMessage) {
    return Effect.subscription(subscriptionId, (dispatch) => {
 ...
 * ```
 */
export function subscribeToEvents(client, subscriptionId, onEvent) {
    return Effect.subscription(subscriptionId, (dispatch) => {
