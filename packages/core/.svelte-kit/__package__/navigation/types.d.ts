 ...
 * ```
 */
export declare const PresentationAction: {
    /**
     ...
     */
    readonly presented: <T>(action: T) => PresentationAction<T>;
    /**
     ...
     */
    readonly dismiss: <T>() => PresentationAction<T>;
};
/**
 ...
 * ```
 */
export declare const StackAction: {
    /**
     ...
     */
    readonly push: <T>(state: unknown) => StackAction<T>;
    /**
     ...
     */
    readonly popToRoot: <T>() => StackAction<T>;
    /**
     ...
     */
    readonly setPath: <T>(path: readonly unknown[]) => StackAction<T>;
    /**
     ...
     */
    readonly screen: <T>(index: number, action: PresentationAction<T>) => StackAction<T>;
};
/**
 ...
 * ```
 */
export type DestinationCasePath<State extends {
    type: string;
}> = Extract<State, {
    type: string;
}>['type'] | (State extends {
    type: infer CaseType extends string;
} ? CaseType extends any ? Extract<State, {
    type: CaseType;
}> extends {
    state: infer S;
} ? S extends {
    _actions: infer Actions;
} ? Actions extends {
    type: infer ActionType extends string;
} ? `${CaseType}.${ActionType}` : never : never : never : never : never);
/**
 ...
 * ```
 */
export type ExtractCaseType<Path extends string> = Path extends `${infer Case}.${string}` ? Case : Path;
/**
 ...
 * ```
 */
export type ExtractActionType<Path extends string> = Path extends `${string}.${infer Action}` ? Action : never;
/**
 ...
 * ```
 */
export type ExtractCaseState<State extends {
    type: string;
    state: any;
}, CaseType extends string> = Extract<State, {
    type: CaseType;
}> extends {
    state: infer S;
} ? S : never;
