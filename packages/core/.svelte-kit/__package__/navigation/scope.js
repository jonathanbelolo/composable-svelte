...
class ScopeBuilder {
    ...
    case(caseType) {
        ...
        if (typeof value !== 'object') {
            if (import.meta.env.DEV) {
                console.warn(`[scopeTo.case] Value at path [${this.path.join('.')}] is not an object (got ${typeof value}). ` +
        ...
        if (!('type' in value)) {
            if (import.meta.env.DEV) {
                console.warn(`[scopeTo.case] Value at path [${this.path.join('.')}] is missing 'type' field. ` +
                    `Expected a discriminated union like { type: '${caseType}', state: ... }`);
            }
            return null;
        }
        if (!('state' in value)) {
            if (import.meta.env.DEV) {
                console.warn(`[scopeTo.case] Value at path [${this.path.join('.')}] is missing 'state' field. ` +
        ...
        if (value.type === caseType) {
            ...
            if (import.meta.env.DEV && state === undefined) {
                console.warn(`[scopeTo.case] Destination at path [${this.path.join('.')}] has case '${caseType}' ` +
    ...
    optional() {
        ...
        if (import.meta.env.DEV &&
            ...
            'type' in value &&
            'state' in value) {
            console.warn(`[scopeTo.optional] Value at path [${this.path.join('.')}] looks like a discriminated union ` +
                `(has 'type' and 'state' fields). Did you mean to use .case('${value.type}') instead of .optional()?`);
        }
     ...
     */
    createScopedStore(state, caseType) {
