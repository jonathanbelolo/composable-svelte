 ...
 * ```
 */
export function push(stack, screenState) {
 ...
 * ```
 */
export function popToRoot(stack) {
    if (stack.length === 0) {
        return [stack, Effect.none()];
    }
 ...
 * ```
 */
export function handleStackAction(state, action, deps, screenReducer, getStack, setStack) {
    const stack = getStack(state);
    switch (action.type) {
        case 'push': {
            const [newStack, effect] = push(stack, action.state);
        ...
        case 'setPath': {
            const [newStack, effect] = setPath(action.path);
            return [setStack(state, newStack), effect];
        }
        case 'screen': {
            ...
            if (index < 0 || index >= stack.length) {
                console.warn(`[Composable Svelte] Invalid stack index: ${index} (stack length: ${stack.length})`);
                return [state, Effect.none()];
            }
            // Handle dismiss action
            if (presentationAction.type === 'dismiss') {
                // Dismissing a screen pops it and all screens above it
                const [newStack, effect] = setPath(stack.slice(0, index));
                return [setStack(state, newStack), effect];
            }
            ...
            if (presentationAction.type === 'presented') {
                ...
                // Run screen reducer
                const [newScreenState, screenEffect] = screenReducer(screenState, screenAction, deps);
                ...
                const parentEffect = Effect.map(screenEffect, (sa) => ({
                    ...
                    action: {
                        type: 'screen',
                        index,
                        action: { type: 'presented', action: sa }
                    }
                }));
        ...
        default: {
            ...
            console.warn(`[Composable Svelte] Unhandled stack action: ${_exhaustive.type}`);
            return [state, Effect.none()];
        }
    }
}
 ...
 * ```
 */
export function topScreen(stack) {
 ...
 */
export function rootScreen(stack) {
 ...
 */
export function canGoBack(stack) {
