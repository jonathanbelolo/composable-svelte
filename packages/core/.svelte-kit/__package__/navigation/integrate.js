 ...
 * ```
 */
export function integrate(coreReducer) {
...
class IntegrationBuilder {
     ...
     * ```
     */
    with(field, childReducer) {
        // Validate that field hasn't already been registered
        if (this.fields.has(field)) {
            throw new Error(`[integrate] Field '${String(field)}' has already been integrated. ` +
                `Each field can only be registered once.`);
        }
        // Validate that childReducer is a function
        if (typeof childReducer !== 'function') {
            throw new TypeError(`[integrate] childReducer for field '${String(field)}' must be a function, ` +
        ...
        this.integrations.push((parentReducer) => {
            return (state, action, deps) => {
                ...
                // Child reducer
                childReducer)(stateAfterParent, action, deps);
