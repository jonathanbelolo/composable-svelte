 ...
 * ```
 */
export function createInitialFormState(config, data) {
 ...
 * ```
 */
export function createFormReducer(config) {
        ...
        switch (action.type) {
            ...
            case 'fieldChanged': {
                ...
                if (mode === 'onChange' || mode === 'all') {
                    ...
                    return [
                        newState,
                        Effect.debounced(`validate-${String(field)}`, // Unique ID per field
                        debounceMs, async (dispatch) => {
            ...
            case 'fieldValidationStarted': {
                ...
                return [
                    newState,
                    Effect.cancellable(`validate-${String(field)}`, // Cancel previous validation for this field
                    async (dispatch) => {
                        ...
                        if (!error && asyncValidators?.[field]) {
                            try {
                                await asyncValidators[field](fieldValue);
                    ...
                    Effect.run(async (dispatch) => {
                        try {
                            ...
                            if (config.onSubmitError) {
                                config.onSubmitError(e instanceof Error ? e : new Error('Submission failed'));
                            }
                        }
                    })
                ];
