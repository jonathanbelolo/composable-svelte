 ...
 * ```
 */
export const accordionReducer = (state, action, deps) => {
    switch (action.type) {
        case 'itemToggled': {
            ...
            if (isExpanded) {
                ...
                if (!state.collapsible && state.expandedIds.length === 1) {
                    return [state, Effect.none()]; // Can't collapse last item
                }
        ...
        case 'allExpanded': {
            ...
            const effects = deps?.onExpand
                ? newlyExpandedIds.map((id) => Effect.run(async () => {
                    deps.onExpand?.(id);
                }))
                : [];
            return [
        ...
        case 'allCollapsed': {
            ...
            const effects = deps?.onCollapse
                ? collapsedIds.map((id) => Effect.run(async () => {
                    deps.onCollapse?.(id);
                }))
                : [];
            return [
